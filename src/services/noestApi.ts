...
export async function createOrder(params: {
  ...
  stop_desk: number;
  station_code?: string;
}): Promise<ApiResponse<{ id: string; tracking: string }>> {
  try {
    // Call our Vercel serverless proxy at /api/noest
    ...
    console.warn('[NOEST] API error:', result.error || result);
    return {
      ok: false,
      error: result.error || 'فشل إرسال الطلب إلى شركة التوصيل',
    };
  } catch (err) {
    // Network error or /api/noest not available (local dev)
    console.warn('[NOEST] Network error, using fallback:', err);

    // Fallback: generate a local tracking number
    const stationPart = params.station_code || `${params.wilaya_id}A`;
    const randomNum = Math.floor(10000000 + Math.random() * 90000000);
    const tracking = `BX4-${stationPart}-${randomNum}`;
