...
export function tryDeterministicAnswer({ message, history, products }) {
  ...
  const shipping = findShippingDeterministic(context);

  // Guided recommendation: never ask unnecessary questions or mix languages.
  if (wantsRecommendation(message)) {
    ...
    if (primary.length === 3) {
      return {
        answer: `للابتدائي عندنا 3 حقائب كاملة:\n${formatPrimaryList(primary)}\nقولي السنة اللي تقريها أو إذا حبيتي السنوات الثلاثة كاملة.`,
        source: 'rules',
      };
    }
  }

  // Bare "ابتدائي" after recommendation.
  if (nMessage === normalizeRuleText('ابتدائي') && primary.length === 3) {
    return {
      answer: `للابتدائي متوفرة:\n${formatPrimaryList(primary)}\nإذا حبيتي السنوات الثلاثة كاملة نحسبهالك مباشرة.`,
      source: 'rules',
    };
  }

  // Specific PS price/link.
  ...
  // Primary full bundle / three years.
  if (primary.length === 3 && (wantsFullPrimary(context) || (isPrimaryContext(context) && nMessage.includes('كامله')))) {
    const subtotal = primary.reduce((sum, p) => sum + Number(p.price || 0), 0);
    const lines = [
      'الحقيبة الكاملة للسنوات الثلاثة:',
      ...primary.map((p) => `• ${p.name.split('|')[0].trim()} — 💰 ${money(p.price)}`),
      `المجموع: 💰 ${money(subtotal)}`,
    ];

    if (shipping && (nMessage.includes('توصيل') || nMessage.includes('شحن') || normalizeRuleText(context).includes('توصيل'))) {
  ...
  // Pure delivery question with a known wilaya/commune.
  if (shipping && (nMessage.includes('توصيل') || nMessage.includes('شحن'))) {
    if (wantsOffice(message)) {
  ...
  // Link request while primary context is active.
