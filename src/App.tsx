        ...
        <div className="fixed inset-0 bg-black/70 z-[8000] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6">
              ...
              {safeArr(selectedProduct.benefits).length > 0 && (<div className="bg-emerald-50 rounded-xl p-4 mb-4"><h4 className="font-bold text-emerald-800 mb-2">✅ الفوائد التعليمية:</h4><ul className="space-y-1">{safeArr(selectedProduct.benefits).map((b, i) => <li key={i} className="text-sm text-emerald-700 flex items-start gap-2"><span>•</span>{b}</li>)}</ul></div>)}
...
function SystemHealthCard({ showToast }: { showToast: (msg: string, type?: 'success' | 'error' | 'info') => void }) {
  ...
  return (
    <div className={`rounded-2xl p-4 border-2 ${!checked ? 'bg-gray-50 border-gray-300' : allOk ? 'bg-green-50 border-green-300' : 'bg-red-50 border-red-300'}`}>
      <div className="flex items-center gap-3 mb-2">
      ...
      {checked && resultText && (
        <pre className="mt-3 bg-gray-900 text-green-400 rounded-xl p-3 text-xs overflow-x-auto max-h-64" dir="ltr">
          {resultText}
        </pre>
      )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {products.map(product => (
                <div key={product.id} className="bg-white rounded-2xl shadow-md overflow-hidden hover:shadow-lg transition-all">
                  <div className="relative h-40"><img src={safeImage(product.images)} alt={product.name} className="w-full h-full object-cover" />{product.badge && <span className="absolute top-2 right-2 bg-amber-500 text-white text-xs px-2 py-1 rounded-full font-bold">{product.badge}</span>}<span className="absolute top-2 left-2 bg-emerald-700 text-white text-xs px-2 py-1 rounded-full font-bold">{product.category}</span></div>
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between flex-wrap gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={openAddLp} className="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 transition-all shadow-md">
                    ➕ إنشاء صفحة
                  </button>
                </div>
              </div>
              ...
              {!lpLoading && !lpError && landingPages.length === 0 && (
                <div className="bg-white rounded-2xl shadow-md p-12 text-center">
                  ...
                  <button onClick={openAddLp} className="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3 rounded-xl font-bold transition-all shadow-md">
                    ➕ إنشاء أول صفحة هبوط
                  </button>
                </div>
              )}
                        ...
                        <div className="flex flex-col sm:flex-row">
                          ...
                          <div className="flex-1 p-4">
                            <div className="flex items-start justify-between gap-3 mb-2">
                              <div>
                                <div className="flex items-center gap-2 flex-wrap mb-1">
                                  ...
                                  <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${lp.is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                                    {lp.is_active ? '✅ نشط' : '⏸️ معطّل'}
                                  </span>
                                </div>
                                <div className="flex items-center gap-2 flex-wrap text-xs text-gray-500">
                                  <span className="bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded font-mono font-bold">/l/{lp.slug}</span>
                                  {linkedProduct && (
                            ...
                            <div className="flex flex-wrap gap-2 mt-2">
                              <button onClick={() => handleToggleLpActive(lp)} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${lp.is_active ? 'bg-amber-50 hover:bg-amber-100 text-amber-700' : 'bg-green-50 hover:bg-green-100 text-green-700'}`}>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 focus:border-emerald-500 outline-none"
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 focus:border-emerald-500 outline-none font-mono text-sm"
              ...
              <div>
                ...
                <div className="flex gap-3 items-start">
                  {/* Preview */}
                  {lpForm.image_url && (
                    <div className="relative w-32 h-20 rounded-xl overflow-hidden border-2 border-emerald-200 flex-shrink-0">
                  ...
                  <div className="flex-1 space-y-2">
                    <input
                      ...
                      className="w-full border-2 border-gray-200 rounded-xl px-4 py-2 focus:border-emerald-500 outline-none text-sm"
                    ...
                    <button
                      ...
                      className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${lpImageUploading ? 'bg-gray-300 text-gray-500' : 'bg-emerald-100 hover:bg-emerald-200 text-emerald-700'}`}
                    >
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 focus:border-emerald-500 outline-none text-sm"
              ...
              <div className="flex items-center gap-3 bg-gray-50 rounded-xl p-4">
                <button
                  ...
                  className={`w-12 h-7 rounded-full transition-all relative ${lpForm.is_active ? 'bg-emerald-500' : 'bg-gray-300'}`}
                >
              ...
              {lpForm.slug && (
                <div className="bg-emerald-50 border-2 border-emerald-200 rounded-xl p-3 flex items-center gap-2">
                  <span className="text-emerald-700 font-bold text-sm">🔗 الرابط:</span>
                  <code className="text-emerald-800 text-xs font-mono bg-emerald-100 px-2 py-1 rounded flex-1" dir="ltr">
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  disabled={lpSaving}
                  className={`flex-1 py-3 rounded-xl font-bold transition-all text-white ${lpSaving ? 'bg-gray-400 cursor-not-allowed' : 'bg-emerald-600 hover:bg-emerald-700'}`}
                >
      ...
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-white flex items-center justify-center" dir="rtl">
        <div className="text-center space-y-6">
          ...
          <div className="flex items-center justify-center gap-3">
            ...
            <span className="text-emerald-700 font-bold text-lg">جاري تحميل المنتجات...</span>
          </div>
