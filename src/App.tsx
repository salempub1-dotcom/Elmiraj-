...
const Toast = ({ message, type, onClose }: { message: string; type: 'success' | 'error' | 'info'; onClose: () => void }) => {
  ...
  const colors = { success: 'bg-blue-500', error: 'bg-red-500', info: 'bg-blue-500' };
  return (
...
function StoreApp({
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans" dir="rtl">
      ...
      {/* Announcement */}
      <div className="bg-blue-800 text-white text-center py-2 text-sm font-medium">🎓 منصة المعراج التعليمية | 🚚 التوصيل متوفر لجميع ولايات الجزائر | 💵 الدفع عند الاستلام</div>

      {/* Header */}
      <header className="bg-white shadow-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3"><Logo size="md" /><div><h1 className="text-xl font-bold text-blue-800">المعراج</h1><p className="text-xs text-blue-600">متجر تعليمي للأساتذة</p></div></div>
          <nav className="hidden md:flex items-center gap-6 text-sm font-bold text-gray-600">
            {[{ id: 'home', label: '🏠 الرئيسية' }, { id: 'products', label: '📚 المنتجات' }, { id: 'track', label: '🔍 تتبع الطلب' }, { id: 'contact', label: '📞 اتصل بنا' }].map(s => (
              <button key={s.id} onClick={() => setActiveSection(s.id)} className={`hover:text-blue-700 transition-colors pb-1 ${activeSection === s.id ? 'text-blue-700 border-b-2 border-blue-700' : ''}`}>{s.label}</button>
            ))}
          </nav>
          <div className="flex items-center gap-3">
            ...
            }} placeholder="ابحث..." className="border-2 border-gray-200 rounded-xl px-4 py-2 text-sm focus:border-blue-500 outline-none w-40" /></div>
            <button onClick={() => setCartOpen(true)} className={`relative bg-blue-700 hover:bg-blue-800 text-white p-3 rounded-xl transition-all ${cartAnimating ? 'animate-bounce' : ''}`}>
              🛒
      ...
      {activeSection === 'home' && (
        <section className="py-12 px-4 bg-white">
          <div className="max-w-6xl mx-auto">
            <h2 className="text-3xl font-bold text-center text-blue-800 mb-8">منصة المعراج ليست مجرد منصة تعليمية، بل شريك نجاح حقيقي لكل أستاذ طموح</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[{ icon: '🎯', title: 'أدوات تفاعلية فعّالة', desc: 'تساعد الأستاذ في تنويع طرق التدريس وجذب انتباه التلاميذ طوال الحصة' }, { icon: '📋', title: 'تنويع طرق التدريس', desc: 'أدوات متنوعة تساعد الأستاذ على تقديم الدروس بأساليب مختلفة تناسب جميع التلاميذ' }, { icon: '🔬', title: 'مبنية على أسس تربوية', desc: 'كل منتج مصمم وفق أحدث الأساليب التربوية لضمان أقصى فائدة تعليمية' }].map((f, i) => (
                <div key={i} className="bg-blue-50 rounded-2xl p-6 text-center hover:shadow-md transition-all"><span className="text-4xl block mb-3">{f.icon}</span><h3 className="text-lg font-bold text-blue-800 mb-2">{f.title}</h3><p className="text-gray-600 text-sm">{f.desc}</p></div>
              ))}
            </div>
          </div>
        </section>
      ...
      {(activeSection === 'home' || activeSection === 'products') && (
        <section className="py-12 px-4 bg-gray-50">
          <div className="max-w-7xl mx-auto">
            <h2 className="text-3xl font-bold text-center text-blue-800 mb-8">الأطوار التعليمية</h2>
            <div className="flex flex-wrap justify-center gap-3 mb-8">
              ...
              }} className={`px-5 py-2.5 rounded-xl font-bold transition-all ${selectedCategory === cat ? 'bg-blue-700 text-white shadow-md' : 'bg-white text-gray-600 hover:bg-blue-50 border border-gray-200'}`}>{catEmoji(cat)} {cat}</button>))}
            </div>
            ...
            }} placeholder="ابحث عن منتج..." className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 focus:border-blue-500 outline-none" /></div>
              ...
              {filteredProducts.map(product => (
                <div key={product.id} className="bg-white rounded-2xl shadow-md hover:shadow-xl transition-all overflow-hidden group">
                  <div className="relative h-48 overflow-hidden cursor-pointer" onClick={() => {
                    ...
                    <span className="absolute top-2 left-2 bg-blue-700 text-white text-xs px-2 py-1 rounded-full font-bold">{product.category}</span>
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between mb-3"><span className="text-blue-700 font-bold text-lg">{product.price.toLocaleString()} دج</span><span className="text-gray-400 text-xs">المخزون: {product.stock}</span></div>
                    <div className="flex gap-2">
                      <button onClick={() => addToCart(product)} className="flex-1 bg-blue-700 hover:bg-blue-800 text-white py-2 rounded-lg font-bold text-xs transition-all">🛒 أضف للعربة</button>
      ...
      {activeSection === 'track' && (
        <section className="py-16 px-4 min-h-[60vh]">
          <div className="max-w-xl mx-auto">
            <h2 className="text-3xl font-bold text-center text-blue-800 mb-8">🔍 تتبع طلبك</h2>
            <div className="bg-white rounded-2xl shadow-xl p-8">
              <label className="block text-sm font-bold text-gray-700 mb-3">أدخل رقم التتبع</label>
              <input type="text" value={trackingInput} onChange={e => setTrackingInput(e.target.value)} placeholder="مثال: BX4-16G-14705085" className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 focus:border-blue-500 outline-none mb-4 text-center font-mono font-bold text-lg" />
              <button onClick={() => { if (!trackingInput.trim()) { showToast('يرجى إدخال رقم التتبع', 'error'); return; } const found = orders.find(o => o.tracking === trackingInput.trim()); if (found) { setTrackingResult(`الحالة: ${found.status === 'pending' ? '⏳ قيد الانتظار' : found.status === 'confirmed' ? '✅ مؤكد' : found.status === 'shipped' ? '🚚 في الطريق' : found.status === 'delivered' ? '📦 تم التوصيل' : '❌ ملغي'}`); } else { window.open(`https://app.noest-dz.com/tracking?code=${trackingInput.trim()}`, '_blank'); setTrackingResult('تم تحويلك لموقع NOEST لمتابعة الشحنة...'); } }} className="w-full bg-blue-700 hover:bg-blue-800 text-white py-3 rounded-xl font-bold text-lg transition-all">🔍 تتبع الطلب</button>
              {trackingResult && <div className="mt-4 bg-blue-50 border-2 border-blue-200 rounded-xl p-4 text-blue-800 font-bold text-center">{trackingResult}</div>}
            </div>
          </div>
        </section>
      )}
      ...
      {activeSection === 'contact' && (
        <section className="py-16 px-4 min-h-[60vh]">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-3xl font-bold text-center text-blue-800 mb-8">📞 اتصل بنا</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="bg-white rounded-2xl shadow-xl p-8 space-y-6">
                <div><h3 className="text-lg font-bold text-gray-700 mb-1">📱 الهاتف</h3><a href="tel:0782272080" className="text-blue-700 font-bold text-xl hover:underline">0782272080</a></div>
                <div><h3 className="text-lg font-bold text-gray-700 mb-1">📧 البريد الإلكتروني</h3><a href="mailto:contact@almiraj.dz" className="text-blue-700 hover:underline">contact@almiraj.dz</a></div>
      ...
      <footer className="bg-blue-900 text-white py-10 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="border-t border-blue-700 pt-6 text-center"><p className="text-blue-400 text-sm">2024 المعراج - جميع الحقوق محفوظة 🇩🇿</p></div>
        </div>
      </footer>
      ...
      {selectedProduct && (
        <div className="fixed inset-0 bg-black/70 z-[8000] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            {safeImages(selectedProduct.images).length > 1 && (<div className="flex gap-2 p-3 overflow-x-auto">{safeImages(selectedProduct.images).map((img, i) => (<img key={i} src={img} alt="" onClick={() => setCurrentImageIndex(i)} className={`h-16 w-16 object-cover rounded-lg cursor-pointer flex-shrink-0 transition-all ${currentImageIndex === i ? 'ring-2 ring-blue-500 scale-105' : 'opacity-60 hover:opacity-100'}`} />))}</div>)}
            <div className="p-6">
              <div className="flex items-start justify-between mb-3"><div><h2 className="text-xl font-bold text-gray-800">{selectedProduct.name}</h2><span className="bg-blue-100 text-blue-700 text-xs px-2 py-1 rounded-full font-bold">{selectedProduct.category}</span></div><span className="text-2xl font-bold text-blue-700">{selectedProduct.price.toLocaleString()} دج</span></div>
              <p className="text-gray-600 mb-4 text-sm">{safeStr(selectedProduct.description)}</p>
              {safeArr(selectedProduct.benefits).length > 0 && (<div className="bg-blue-50 rounded-xl p-4 mb-4"><h4 className="font-bold text-blue-800 mb-2">✅ الفوائد التعليمية:</h4><ul className="space-y-1">{safeArr(selectedProduct.benefits).map((b, i) => <li key={i} className="text-sm text-blue-700 flex items-start gap-2"><span>•</span>{b}</li>)}</ul></div>)}
              <div className="flex gap-3"><button onClick={() => { addToCart(selectedProduct); setSelectedProduct(null); }} className="flex-1 bg-blue-700 hover:bg-blue-800 text-white py-3 rounded-xl font-bold transition-all">🛒 أضف للعربة</button><button onClick={() => { buyNow(selectedProduct); setSelectedProduct(null); }} className="flex-1 bg-amber-500 hover:bg-amber-600 text-white py-3 rounded-xl font-bold transition-all">⚡ اشتري الآن</button></div>
            </div>
          </div>
        </div>
      )}
      ...
      {cartOpen && (
        <div className="fixed inset-0 z-[8000]">
          ...
          <div className="absolute left-0 top-0 h-full w-full max-w-md bg-white shadow-2xl flex flex-col">
            <div className="bg-blue-700 text-white px-6 py-4 flex justify-between items-center"><h2 className="font-bold text-xl">🛒 سلة التسوق ({cartCount})</h2><button onClick={() => setCartOpen(false)} className="text-white hover:text-gray-200 text-2xl font-bold">✕</button></div>
            ...
            {cart.length > 0 && (<div className="border-t p-4 space-y-3"><div className="flex justify-between font-bold text-lg"><span>المجموع:</span><span className="text-blue-700">{cartTotal.toLocaleString()} دج</span></div><p className="text-xs text-gray-500 text-center">🚚 التوصيل متوفر لجميع ولايات الجزائر | 💵 الدفع عند الاستلام</p><button onClick={() => {
                  ...
                  fbTrack('InitiateCheckout', buildCartCatalogData(cart));
                }} className="w-full bg-blue-700 hover:bg-blue-800 text-white py-3 rounded-xl font-bold text-lg transition-all">✅ إتمام الطلب</button></div>)}
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[95vh] overflow-y-auto">
            {orderPlaced && currentOrder ? (
              <div className="p-8 text-center">
                ...
                <h2 className="text-2xl font-bold text-blue-700 mb-2">تم تأكيد طلبك!</h2>
                ...
                <div className="bg-blue-50 border-2 border-blue-300 rounded-2xl p-6 mb-6">
                  ...
                  <p className="text-2xl font-mono font-bold text-blue-700 mb-3">{currentOrder.tracking}</p>
                  <button onClick={() => copyTracking(currentOrder.tracking)} className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${copiedTracking ? 'bg-blue-500 text-white' : 'bg-blue-100 text-blue-700 hover:bg-blue-200'}`}>{copiedTracking ? '✅ تم النسخ!' : '📋 نسخ رقم التتبع'}</button>
                </div>
                ...
                <div className="flex gap-3"><button onClick={() => { setActiveSection('track'); setTrackingInput(currentOrder.tracking); resetCheckout(); }} className="flex-1 border-2 border-blue-600 text-blue-700 py-3 rounded-xl font-bold hover:bg-blue-50 transition-all">🔍 تتبع الطلب</button><button onClick={resetCheckout} className="flex-1 bg-blue-700 text-white py-3 rounded-xl font-bold hover:bg-blue-800 transition-all">🏠 العودة للمتجر</button></div>
              </div>
            ) : (
              <>
                <div className="bg-blue-700 text-white px-6 py-4 flex justify-between items-center"><h2 className="font-bold text-lg">📦 إتمام الطلب</h2><button onClick={resetCheckout} className="text-white hover:text-gray-200 text-xl font-bold">✕</button></div>
                <div className="p-6 space-y-4">
                  <div><label className="block text-sm font-bold text-gray-700 mb-1">الاسم الكامل *</label><input type="text" value={customerName} onChange={e => setCustomerName(e.target.value)} className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 focus:border-blue-500 outline-none" placeholder="أدخل اسمك الكامل" /></div>
                  <div><label className="block text-sm font-bold text-gray-700 mb-1">رقم الهاتف * (10 أرقام)</label><input type="tel" value={customerPhone} onChange={e => { const v = e.target.value.replace(/\D/g, ''); if (v.length <= 10) setCustomerPhone(v); }} className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 focus:border-blue-500 outline-none" placeholder="05XXXXXXXX" /><p className={`text-xs mt-1 ${customerPhone.length === 10 ? 'text-blue-500 font-bold' : 'text-gray-400'}`}>{customerPhone.length}/10</p></div>
                  <div><label className="block text-sm font-bold text-gray-700 mb-1">الولاية * <span className="text-xs text-gray-400 font-normal">(Wilaya)</span></label><select value={customerWilayaId === '' ? '' : String(customerWilayaId)} onChange={e => { const v = e.target.value; if (!v) { setCustomerWilayaId(''); setCustomerWilayaLabel(''); setNoestCommunes([]); setCommune(''); setSelectedOffice(''); return; } const id = Number(v); setCustomerWilayaId(id); const w = noestWilayas.find(x => x.code === id); setCustomerWilayaLabel(w ? `${w.code} - ${w.nom} (${w.nom_ar})` : String(id)); setSelectedOffice(''); }} className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 focus:border-blue-500 outline-none"><option value="">اختر الولاية / Choisir la wilaya</option>{noestWilayas.length > 0 ? noestWilayas.slice().sort((a, b) => a.code - b.code).map(w => (<option key={w.code} value={String(w.code)}>{w.code} - {w.nom} ({w.nom_ar})</option>)) : wilayaShipping.slice().sort((a, b) => a.code - b.code).map(w => (<option key={w.code} value={String(w.code)}>{w.code} - {w.name}</option>))}</select></div>
                  <div><label className="block text-sm font-bold text-gray-700 mb-1">البلدية * <span className="text-xs text-gray-400 font-normal">(Commune)</span></label><select value={commune} onChange={e => setCommune(e.target.value)} className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 focus:border-blue-500 outline-none" disabled={!customerWilayaId || loadingNoest}><option value="">{!customerWilayaId ? 'اختر الولاية أولاً' : loadingNoest ? 'جاري تحميل البلديات...' : 'اختر البلدية / Choisir la commune'}</option>{noestCommunes.map((c, idx) => (<option key={`${c.wilaya_id}-${idx}`} value={c.nom}>{c.nom} ({c.nom_ar})</option>))}</select></div>
                  <div><label className="block text-sm font-bold text-gray-700 mb-1">العنوان التفصيلي *</label><textarea value={customerAddress} onChange={e => setCustomerAddress(e.target.value)} className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 focus:border-blue-500 outline-none" rows={2} placeholder="أدخل عنوانك التفصيلي" /></div>
                  <div><label className="block text-sm font-bold text-gray-700 mb-2">نوع التوصيل *</label><div className="grid grid-cols-2 gap-3">{[{ value: 'home', icon: '🏠', label: 'إلى المنزل' }, { value: 'office', icon: '🏢', label: 'إلى المكتب' }].map(opt => (<button key={opt.value} onClick={() => { setDeliveryType(opt.value as 'home' | 'office'); setSelectedOffice(''); }} className={`p-3 rounded-xl border-2 font-bold text-sm transition-all ${deliveryType === opt.value ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-600 hover:border-blue-300'}`}>{opt.icon} {opt.label}</button>))}</div></div>
                  {deliveryType === 'office' && customerWilayaId && (<div><label className="block text-sm font-bold text-gray-700 mb-2">🏢 اختر مكتب الاستلام * <span className="text-xs text-gray-400 font-normal">(Stop Desk)</span></label>{desks.length > 0 ? (<div className="max-h-48 overflow-y-auto space-y-2 border-2 border-gray-200 rounded-xl p-3">{desks.map(desk => (<button key={desk.code} onClick={() => setSelectedOffice(`${desk.code} — ${desk.name}`)} className={`w-full flex items-center gap-3 p-3 rounded-xl text-right transition-all border-2 ${selectedOffice === `${desk.code} — ${desk.name}` ? 'border-blue-500 bg-blue-50' : 'border-gray-100 hover:border-blue-300'}`}><span className="bg-blue-700 text-white text-xs px-2 py-1 rounded-lg font-mono font-bold">{desk.code}</span><div className="flex-1"><span className="font-bold text-gray-800 text-sm block">{desk.name_ar}</span><span className="text-gray-500 text-xs">{desk.name}</span></div>{selectedOffice === `${desk.code} — ${desk.name}` && <span className="text-blue-500 mr-auto font-bold">✓</span>}</button>))}</div>) : (<div className="bg-amber-50 border-2 border-amber-200 rounded-xl p-4 text-amber-700 text-sm font-bold text-center">⚠️ سيتم التواصل معك لتحديد نقطة الاستلام</div>)}</div>)}
                  <div className="bg-gray-50 rounded-xl p-4 space-y-2">
                    ...
                    <div className="flex items-center gap-2 bg-blue-50 rounded-xl p-3 mt-2"><span>💵</span><span className="text-blue-700 font-bold text-sm">الدفع عند الاستلام</span></div>
                  </div>
                  {orderError && (
                  ...
                  <button onClick={handlePlaceOrder} disabled={placingOrder} className={`w-full py-4 rounded-xl font-bold text-lg transition-all text-white ${placingOrder ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-700 hover:bg-blue-800 shadow-lg'}`}>{placingOrder ? '⏳ جاري إرسال الطلب لشركة التوصيل...' : orderError ? '🔄 إعادة المحاولة' : '✅ تأكيد الطلب'}</button>
                </div>
              </>
            )}
          </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex md:hidden z-40">
        {[{ id: 'home', icon: '🏠', label: 'الرئيسية' }, { id: 'products', icon: '📚', label: 'المنتجات' }, { id: 'track', icon: '🔍', label: 'تتبع' }, { id: 'cart', icon: '🛒', label: `(${cartCount})` }, { id: 'contact', icon: '📞', label: 'تواصل' }].map(item => (
          <button key={item.id} onClick={() => item.id === 'cart' ? setCartOpen(true) : setActiveSection(item.id)} className={`flex-1 flex flex-col items-center py-2 text-xs font-bold transition-colors ${activeSection === item.id ? 'text-blue-700' : 'text-gray-400 hover:text-blue-600'}`}><span className="text-lg">{item.icon}</span><span>{item.label}</span></button>
        ))}
      </nav>
  ...
  return (
    <div className={`rounded-2xl p-4 border-2 ${statusColors[status]}`}>
      <div className="flex items-center gap-3 mb-2">
        ...
        <button onClick={runDiagnose} className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-lg text-xs font-bold transition-all whitespace-nowrap">🔍 فحص</button>
      </div>
      {status === 'demo' && (
...
function AdminApp({
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-900 via-blue-800 to-blue-700 flex items-center justify-center p-4" dir="rtl">
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-md">
          ...
          <div className="space-y-5">
            <div><label className="block text-sm font-bold text-gray-700 mb-2">👤 اسم المستخدم</label><input type="text" value={adminUsername} onChange={e => setAdminUsername(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleLogin()} className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 focus:border-blue-500 outline-none text-lg" placeholder="admin" /></div>
            <div><label className="block text-sm font-bold text-gray-700 mb-2">🔒 كلمة المرور</label><input type="password" value={adminPassword} onChange={e => setAdminPassword(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleLogin()} className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 focus:border-blue-500 outline-none text-lg" placeholder="••••••••" /></div>
            ...
            <button onClick={handleLogin} disabled={loginLoading} className={`w-full py-4 rounded-xl font-bold text-lg transition-all shadow-lg text-white ${loginLoading ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-700 hover:bg-blue-800'}`}>{loginLoading ? '⏳ جاري التحقق...' : '🔐 دخول'}</button>
          ...
          <div className="mt-6 bg-blue-50 rounded-xl p-4 text-center"><p className="text-xs text-blue-600">🔒 الوصول مخصص للمسؤولين فقط</p></div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-100" dir="rtl">
      ...
      {/* Header */}
      <header className="bg-blue-800 text-white px-4 py-3 flex items-center justify-between shadow-lg fixed top-0 left-0 right-0 z-50">
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            <button onClick={() => setShowNotif(!showNotif)} className="relative bg-blue-700 hover:bg-blue-600 p-2 rounded-xl transition-all">🔔{unread > 0 && <span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center font-bold">{unread}</span>}</button>
            {showNotif && (<div className="absolute left-0 top-12 w-72 bg-white rounded-xl shadow-2xl border z-50"><div className="bg-blue-800 text-white px-4 py-3 font-bold flex justify-between rounded-t-xl"><span>الإشعارات</span><button onClick={() => setNotifications(prev => prev.map(n => ({ ...n, read: true })))} className="text-xs text-blue-200 hover:text-white">تعيين الكل كمقروء</button></div><div className="max-h-64 overflow-y-auto">{notifications.length === 0 ? <p className="text-center text-gray-400 py-6 text-sm">لا توجد إشعارات</p> : notifications.map(n => (<div key={n.id} className={`px-4 py-3 border-b text-sm ${n.read ? 'bg-white text-gray-500' : 'bg-blue-50 text-blue-800 font-bold'}`}>{n.message}</div>))}</div></div>)}
          </div>
          <button onClick={onBackToStore} className="bg-blue-700 hover:bg-blue-600 px-3 py-2 rounded-xl text-sm transition-all font-bold">🏪 المتجر</button>
        ...
        <main className="flex-1 md:mr-56 p-4 md:p-6">
          ...
          <div className="flex md:hidden gap-2 mb-4 overflow-x-auto">
            {[{ id: 'dashboard' as const, label: '📊 لوحة' }, { id: 'orders' as const, label: '📋 الطلبات' }, { id: 'products' as const, label: '📦 المنتجات' }, { id: 'landing' as const, label: '🚀 هبوط' }].map(t => (
              <button key={t.id} onClick={() => setTab(t.id)} className={`px-4 py-2 rounded-xl font-bold text-sm whitespace-nowrap transition-all ${tab === t.id ? 'bg-blue-600 text-white' : 'bg-white text-gray-600'}`}>{t.label}</button>
            ))}
          </div>
          ...
          {tab === 'orders' && (<div className="space-y-4">
            ...
            {orders.length === 0 ? <div className="bg-white rounded-2xl shadow-md p-12 text-center"><p className="text-6xl mb-4">📭</p><p className="text-gray-400 text-lg">لا توجد طلبات بعد</p></div> : orders.map(order => (
              <div key={order.id} className="bg-white rounded-2xl shadow-md p-5">
                <div className="flex flex-wrap items-start justify-between gap-3 mb-3"><div><div className="flex items-center gap-2 mb-1 flex-wrap"><span className="font-mono text-blue-700 font-bold">{order.tracking}</span>{order.noestId && <span className="bg-blue-100 text-blue-700 text-xs px-2 py-0.5 rounded-full font-bold">✅ NOEST</span>}</div><p className="text-gray-600 text-sm">👤 {order.customer} | 📞 {order.phone}</p><p className="text-gray-600 text-sm">📍 {order.wilaya} - {order.address}</p><p className="text-gray-600 text-sm">🚚 {order.deliveryType === 'home' ? 'توصيل للمنزل' : `مكتب: ${order.selectedOffice || ''}`}</p></div><div className="text-left"><p className="text-xl font-bold text-blue-700">{order.total.toLocaleString()} دج</p><p className="text-gray-400 text-xs">{order.date}</p></div></div>
          ...
          {tab === 'products' && (<div className="space-y-6">
            <div className="flex items-center justify-between"><h2 className="text-2xl font-bold text-gray-800">📦 إدارة المنتجات</h2><button onClick={openAddProduct} className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 transition-all shadow-md">➕ إضافة منتج</button></div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {products.map(product => (
                <div key={product.id} className="bg-white rounded-2xl shadow-md overflow-hidden hover:shadow-lg transition-all">
                  <div className="relative h-40"><img src={safeImage(product.images)} alt={product.name} className="w-full h-full object-cover" />{product.badge && <span className="absolute top-2 right-2 bg-amber-500 text-white text-xs px-2 py-1 rounded-full font-bold">{product.badge}</span>}<span className="absolute top-2 left-2 bg-blue-700 text-white text-xs px-2 py-1 rounded-full font-bold">{product.category}</span></div>
                  <div className="p-4"><h3 className="font-bold text-gray-800 mb-1 text-sm">{product.name}</h3><div className="flex items-center justify-between mb-3"><span className="text-blue-700 font-bold">{product.price.toLocaleString()} دج</span><span className="text-gray-400 text-xs">مخزون: {product.stock}</span></div><div className="flex gap-2"><button onClick={() => openEditProduct(product)} className="flex-1 bg-blue-50 hover:bg-blue-100 text-blue-700 py-2 rounded-lg font-bold text-sm transition-all">✏️ تعديل</button><button onClick={() => setDeleteConfirm(product.id)} className="flex-1 bg-red-50 hover:bg-red-100 text-red-700 py-2 rounded-lg font-bold text-sm transition-all">🗑️ حذف</button></div></div>
                </div>
              ))}
            </div>
          </div>)}
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between flex-wrap gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={openAddLp} className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 transition-all shadow-md">
                    ➕ إنشاء صفحة
                  </button>
                </div>
              </div>
              ...
              {!lpLoading && !lpError && landingPages.length === 0 && (
                <div className="bg-white rounded-2xl shadow-md p-12 text-center">
                  ...
                  <button onClick={openAddLp} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-bold transition-all shadow-md">
                    ➕ إنشاء أول صفحة هبوط
                  </button>
                </div>
              )}
                        ...
                        <div className="flex flex-col sm:flex-row">
                          ...
                          <div className="flex-1 p-4">
                            <div className="flex items-start justify-between gap-3 mb-2">
                              <div>
                                ...
                                <div className="flex items-center gap-2 flex-wrap text-xs text-gray-500">
                                  <span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded font-mono font-bold">/l/{lp.slug}</span>
                                  {linkedProduct && (
                            ...
                            <div className="flex flex-wrap gap-2 mt-2">
                              ...
                              <button onClick={() => openEditLp(lp)} className="bg-blue-50 hover:bg-blue-100 text-blue-700 px-3 py-1.5 rounded-lg text-xs font-bold transition-all">
                                ✏️ تعديل
                              </button>
              ...
              {!lpLoading && landingPages.length > 0 && (
                <div className="bg-blue-50 border-2 border-blue-200 rounded-2xl p-4">
                  <h4 className="font-bold text-blue-800 text-sm mb-2">📋 كيف تستخدم صفحات الهبوط في حملاتك:</h4>
      ...
      {showProductForm && (
        <div className="fixed inset-0 bg-black/60 z-[9000] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="bg-blue-700 text-white px-6 py-4 flex justify-between items-center"><h3 className="text-lg font-bold">{editingProduct ? '✏️ تعديل المنتج' : '➕ إضافة منتج جديد'}</h3><button onClick={() => setShowProductForm(false)} className="text-white hover:text-gray-200 text-xl">✕</button></div>
            <div className="p-6 space-y-4">
              <div><label className="block text-sm font-bold text-gray-700 mb-2">📸 صور المنتج (حتى 6 صور) {isSupabaseConfigured() ? <span className="text-blue-500 text-xs font-normal">🔒 رفع آمن عبر الخادم</span> : <span className="text-amber-500 text-xs font-normal">💾 محلي فقط</span>}</label><div className="grid grid-cols-3 gap-3 mb-3">{productImages.map((img, i) => (<div key={i} className="relative group aspect-square rounded-xl overflow-hidden border-2 border-blue-200"><img src={img} alt="" className="w-full h-full object-cover" />{i === 0 && <span className="absolute top-1 right-1 bg-blue-600 text-white text-xs px-1.5 py-0.5 rounded-full">رئيسية</span>}{isSupabaseUrl(img) && <span className="absolute bottom-1 right-1 bg-blue-500 text-white text-xs px-1 py-0.5 rounded-full">☁️</span>}<button onClick={() => handleRemoveImage(i)} className="absolute top-1 left-1 bg-red-500 text-white w-6 h-6 rounded-full opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center text-sm">✕</button></div>))}{Object.entries(uploadProgress).map(([key, percent]) => (<div key={key} className="aspect-square rounded-xl border-2 border-blue-300 bg-blue-50 flex flex-col items-center justify-center gap-2"><div className="w-10 h-10 border-4 border-blue-200 border-t-blue-600 rounded-full animate-spin" /><span className="text-xs font-bold text-blue-700">{percent}%</span></div>))}{!uploadingImages && productImages.length < 6 && <button onClick={() => fileInputRef.current?.click()} className="aspect-square rounded-xl border-2 border-dashed border-blue-300 hover:border-blue-500 flex flex-col items-center justify-center gap-2 text-blue-500 hover:bg-blue-50 transition-all"><span className="text-2xl">{isSupabaseConfigured() ? '🔒' : '+'}</span><span className="text-xs font-bold">{isSupabaseConfigured() ? 'رفع آمن' : 'رفع صورة'}</span></button>}</div><input ref={fileInputRef} type="file" accept="image/*" multiple className="hidden" onChange={handleImageUpload} /></div>
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2"><label className="block text-sm font-bold text-gray-700 mb-1">اسم المنتج *</label><input type="text" value={productForm.name} onChange={e => setProductForm(p => ({ ...p, name: e.target.value }))} className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 focus:border-blue-500 outline-none" placeholder="اسم المنتج" /></div>
                <div><label className="block text-sm font-bold text-gray-700 mb-1">السعر (دج) *</label><input type="number" value={productForm.price} onChange={e => setProductForm(p => ({ ...p, price: e.target.value }))} className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 focus:border-blue-500 outline-none" placeholder="1500" /></div>
                <div><label className="block text-sm font-bold text-gray-700 mb-1">المخزون *</label><input type="number" value={productForm.stock} onChange={e => setProductForm(p => ({ ...p, stock: e.target.value }))} className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 focus:border-blue-500 outline-none" placeholder="50" /></div>
                <div><label className="block text-sm font-bold text-gray-700 mb-1">الطور الدراسي</label><select value={productForm.category} onChange={e => setProductForm(p => ({ ...p, category: e.target.value as Product['category'] }))} className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 focus:border-blue-500 outline-none"><option value="تحضيري">تحضيري</option><option value="ابتدائي">ابتدائي</option><option value="متوسط">متوسط</option></select></div>
                <div><label className="block text-sm font-bold text-gray-700 mb-1">شارة (اختياري)</label><input type="text" value={productForm.badge} onChange={e => setProductForm(p => ({ ...p, badge: e.target.value }))} className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 focus:border-blue-500 outline-none" placeholder="الأكثر مبيعاً" /></div>
                <div className="col-span-2"><label className="block text-sm font-bold text-gray-700 mb-1">الوصف</label><textarea value={productForm.description} onChange={e => setProductForm(p => ({ ...p, description: e.target.value }))} className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 focus:border-blue-500 outline-none" rows={3} placeholder="وصف المنتج..." /></div>
                <div className="col-span-2"><label className="block text-sm font-bold text-gray-700 mb-1">الفوائد التعليمية (كل فائدة في سطر)</label><textarea value={productForm.benefits} onChange={e => setProductForm(p => ({ ...p, benefits: e.target.value }))} className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 focus:border-blue-500 outline-none" rows={3} placeholder={"فائدة 1\nفائدة 2\nفائدة 3"} /></div>
              </div>
              <div className="flex gap-3 pt-2"><button onClick={handleSaveProduct} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-bold transition-all">💾 {editingProduct ? 'حفظ التعديلات' : 'إضافة المنتج'}</button><button onClick={() => setShowProductForm(false)} className="flex-1 border-2 border-gray-200 text-gray-600 py-3 rounded-xl font-bold hover:bg-gray-50 transition-all">إلغاء</button></div>
            </div>
          </div>
        </div>
      )}
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 focus:border-blue-500 outline-none font-mono text-sm"
                ...
                <div className="flex gap-3 items-start">
                  ...
                  <div className="flex-1 space-y-2">
                    <input
                      ...
                      className="w-full border-2 border-gray-200 rounded-xl px-4 py-2 focus:border-blue-500 outline-none text-sm"
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 focus:border-blue-500 outline-none text-sm"
              ...
              <div className="flex items-center gap-3 bg-gray-50 rounded-xl p-4">
                <button
                  ...
                  className={`w-12 h-7 rounded-full transition-all relative ${lpForm.is_active ? 'bg-blue-500' : 'bg-gray-300'}`}
                >
              ...
              {lpForm.slug && (
                <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-3 flex items-center gap-2">
                  <span className="text-blue-700 font-bold text-sm">🔗 الرابط:</span>
                  <code className="text-blue-800 text-xs font-mono bg-blue-100 px-2 py-1 rounded flex-1" dir="ltr">
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  disabled={lpSaving}
                  className={`flex-1 py-3 rounded-xl font-bold transition-all text-white ${lpSaving ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`}
                >
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-white flex items-center justify-center" dir="rtl">
        <div className="text-center space-y-6">
          ...
          <div className="flex items-center justify-center gap-3">
            ...
            <span className="text-blue-700 font-bold text-lg">جاري تحميل المنتجات...</span>
          </div>
