...
import { trackViewContent, trackInitiateCheckout } from './services/facebook-pixel';
import { Filter, X, ArrowRight } from 'lucide-react';

// Convert Supabase row to Product type
function dbToProduct(row: any): Product {
  return {
    id: row.id_num || Math.random(),
    name: row.name || '',
    description: row.description || '',
    longDescription: row.long_description || row.description || '',
    price: row.price || 0,
    originalPrice: row.original_price || undefined,
    image: row.image_url || row.image_emoji || '📦',
    category: row.category || 'flashcards',
    grade: row.grade || 'preparatory',
    year: row.year || undefined,
    language: row.language || 'english',
    cardCount: row.card_count || 30,
...
function AppContent() {
    ...
    const loadProducts = async () => {
      try {
        ...
        if (data && data.length > 0) {
          setDynamicProducts(data.map(dbToProduct));
          setUsingSupabase(true);
        }
      } catch (err) {
        console.log('Using local products (Supabase not available):', err);
      }
    };
  ...
  // Use dynamic products if available, otherwise local
  const allProducts = usingSupabase ? dynamicProducts : localProducts;
  ...
  // View product details + FB pixel
  const handleViewDetails = useCallback((product: Product) => {
    setSelectedProduct(product);
    trackViewContent(product.name, product.price, product.grade);
  }, []);
  ...
  }, [allProducts, selectedGrade, selectedYear, selectedLanguage]);
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <CartSidebar onCheckout={navigateToCheckout} />
      {selectedProduct && (
        <ProductModal
      ...
      {currentView === 'products' && (
        <section className="py-10">
          <div className="max-w-7xl mx-auto px-4">
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${
                    !selectedGrade ? 'bg-royal-600 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-royal-50 hover:text-royal-600'
                  }`}
                >
                  الكل
                </button>
                {grades.map(grade => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${
                      selectedGrade === grade.id ? 'bg-royal-600 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-royal-50 hover:text-royal-600'
                    }`}
                  >
                ...
                <>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      onClick={() => setSelectedYear(null)}
                      className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${
                        !selectedYear ? 'bg-royal-500 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-royal-50 hover:text-royal-600'
                      }`}
                    >
                      كل السنوات
                    </button>
                    {availableYears.map(yr => (
                      <button
                        ...
                        className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${
                          selectedYear === yr.year ? 'bg-royal-500 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-royal-50 hover:text-royal-600'
                        }`}
                      >
                        {yr.label}
                      </button>
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${
                      selectedLanguage === lang.id ? 'bg-gold-500 text-royal-900 shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gold-50 hover:text-gold-700'
                    }`}
                  >
                    {lang.label}
                  </button>
            ...
            <p className="text-sm text-gray-500 mb-6">عرض {filteredProducts.length} منتج</p>
