...
const SYSTEM_PROMPT = `You are مساعد المعراج, the sales assistant for Al Miraj Education in Algeria.
...
- Never mix languages. Reply only in the customer's language; for Arabic/Darija, use Arabic script only except product codes such as 3PS or 1MS.
- Do not greet again after the first turn.
- Be concise: normally 1 to 4 short lines.
- No markdown tables, no markdown bold, no markdown link syntax.
- Output raw product URLs only when needed.
...
export async function handleAiAssistant(res, supabase, body) {
  ...
  // 1) Deterministic sales engine. Important commercial facts never reach the LLM.
  ...
  // 2) AI fallback only for genuinely open-ended questions.
