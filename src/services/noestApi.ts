...
export async function createOrder(params: CreateOrderParams): Promise<CreateOrderResult> {
  ...
  try {
    ...
    return {
      ok: false,
      error: 'فشل الاتصال بالخادم. تأكد من أن التطبيق منشور على Vercel.',
      debug: msg,
    };
  }
  ...
  console.log(`[NOEST] HTTP ${response.status} — ${rawText.substring(0, 300)}`);

  // ── Step 3: Handle 404 = serverless not available (Arena/local) ──
  if (response.status === 404) {
    // Check if we're getting an HTML 404 page (Arena) vs JSON 404 (NOEST endpoint not found)
    const isHtml = rawText.includes('<html') || rawText.includes('<!DOCTYPE') || rawText.includes('Not Found');
    if (isHtml) {
      console.error('[NOEST] ❌ /api/noest returned HTML 404 — NOT on Vercel');
      return {
        ok: false,
        error: '⚠️ خدمة التوصيل غير متوفرة في هذه البيئة. يجب نشر التطبيق على Vercel لإرسال الطلبات.',
        debug: 'Arena/local dev does not support serverless functions. Deploy to Vercel.',
      };
    }
  }
  ...
  let result: {
    ok?: boolean;
    data?: { id?: string; tracking?: string; endpoint_used?: string };
    error?: string;
    debug?: unknown;
    fix?: string;
  };
  ...
  try {
    ...
    console.error('[NOEST] ❌ Non-JSON response');
    return {
      ok: false,
  ...
  // ── Step 5: Server said ok:false → propagate error ──
  if (result.ok === false) {
    const errorMsg = result.error || `فشل إرسال الطلب (HTTP ${response.status})`;
    console.error('[NOEST] ❌ Server returned ok:false:', errorMsg);
    if (result.debug) console.error('[NOEST] Debug:', JSON.stringify(result.debug));
    return {
      ok: false,
      error: errorMsg,
      debug: result.fix || (typeof result.debug === 'string' ? result.debug : JSON.stringify(result.debug || '').substring(0, 300)),
    };
  }

  // ── Step 6: Server said ok:true → verify tracking/id exists ──
  if (result.ok === true && result.data) {
    const id = result.data.id || '';
    const tracking = result.data.tracking || '';

    if (!id && !tracking) {
      console.error('[NOEST] ❌ ok:true but no tracking/id:', result.data);
      return {
        ok: false,
        error: 'شركة التوصيل لم تُرجع رقم تتبع أو معرف طلب',
      };
    }

    console.log(`[NOEST] ✅ CONFIRMED — tracking=${tracking}, id=${id}, via ${result.data.endpoint_used || '?'}`);
    return {
      ok: true,
      data: { id, tracking },
    };
  ...
  // ── Step 7: Unexpected response shape ──
  console.error('[NOEST] ❌ Unexpected shape:', result);
  return {
    ok: false,
    error: `استجابة غير متوقعة (HTTP ${response.status})`,
    debug: JSON.stringify(result).substring(0, 200),
  };
}
