// ============================================================
// Delivery provider storefront settings
// ============================================================
// Persisted in the existing landing_pages table as one RESERVED system row.
// This avoids a schema migration while still giving the admin a durable toggle
...
export async function writeDeliverySettings(supabase, next) {
  ...
  const row = {
    title: '__SYSTEM__: Delivery providers',
    slug: RESERVED_SLUG,
    product_id: null,
    headline: null,
  ...
  try {
    ...
    if (error) {
      ...
      return { ok: false, error: error.code || 'SETTINGS_SAVE_FAILED', message: 'تعذر حفظ إعدادات شركات التوصيل.' };
    }
    return { ok: true, data: settings };
  } catch (e) {
    console.error('[DELIVERY_SETTINGS] write exception:', e?.message || String(e));
    return { ok: false, error: 'SETTINGS_SAVE_FAILED', message: 'تعذر حفظ إعدادات شركات التوصيل.' };
  }
}
