...
function flattenValidationErrors(value, path = '', out = [], depth = 0) {
...
function normalizeZrWorkflowToken(value) {
  return String(value ?? '')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
...
export async function createZrShipment(order, { sourceHubId = null, pickupHubId = null } = {}) {
  ...
  const orderedProducts = items.length > 0
    ...
    : [{ productId: randomUUID(), productName: 'Produit', productSku: sanitizeZrProductSku(null, `ALMIRAJ-${order.tracking || order.id || 'ORDER'}-01`), unitPrice: Number(order.total) || 0, quantity: 1, length: 1, width: 1, height: 1, weight: 1, stockType: 'local' }];

  const zrPhone = normalizeAlgerianPhone(order.phone);
  ...
  const payload = {
    ...
    customer: {
      ...
      phone: { number1: zrPhone, number2: null, number3: null },
    },
    deliveryAddress: {
    ...
    weight: { weight: 1, dimensionalWeight: null },
