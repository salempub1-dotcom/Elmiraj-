...
function StoreApp({
  ...
  const handlePlaceOrder = async () => {
    ...
    let noestTracking: string | undefined;
    try {
      ...
      if (result.ok && result.data) {
        noestId = result.data.id || undefined;
        noestTracking = result.data.tracking || undefined;
        showToast('✅ تم إرسال الطلب إلى شركة التوصيل بنجاح!', 'success');
      } else {
        console.warn('NOEST API returned error:', result.error);
        showToast('⚠️ تم حفظ الطلب محلياً، سيتم إرساله لاحقاً', 'info');
      }
    } catch (err) {
      console.warn('NOEST API error:', err);
      showToast('⚠️ تم حفظ الطلب، سيتم التواصل معك قريباً', 'info');
    }

    const finalTracking = noestTracking || noestId || trackingNum;
    const newOrder: Order = { id: `ORD-${Date.now()}`, tracking: finalTracking, customer: customerName, phone: customerPhone, wilaya: customerWilayaLabel || String(customerWilayaId), address: customerAddress, items: [...cart], total: orderTotal, shipping: shippingCost, deliveryType, selectedOffice: selectedOffice || undefined, status: 'pending', date: new Date().toLocaleDateString('ar-DZ'), noestId };
    setOrders(prev => [newOrder, ...prev]);
    setCurrentOrder(newOrder);
    setOrderPlaced(true);
    setPlacingOrder(false);
    setCart([]);
    setNotifications(prev => [{ id: Date.now(), message: `طلب جديد من ${customerName} - ${customerWilayaLabel}`, read: false }, ...prev]);
