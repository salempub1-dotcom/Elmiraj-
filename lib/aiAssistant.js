...
export async function handleAiAssistant(res, supabase, body) {
  ...
  try {
    const response = await fetch(NVIDIA_URL, {
      ...
      body: JSON.stringify({ model: NVIDIA_MODEL, messages, temperature: 0.6, max_tokens: 450, stream: false }),
    });
    const data = await response.json().catch(() => ({}));
    if (!response.ok) {
      const providerMessage = String(data?.detail || data?.message || data?.error?.message || '').slice(0, 300);
    ...
    if (!answer) return res.status(502).json({ ok: false, error: 'Empty AI response', model: NVIDIA_MODEL });
