...
export async function createOrder(params: CreateOrderParams): Promise<CreateOrderResult> {
  ...
  if (proxy) {
    ...
    console.log('[NOEST] 🚀 Sending order to NOEST API...');
    const result = await callProxy<{ tracking?: string; id?: string; reference?: string }>('create_order', params as unknown as Record<string, unknown>);

    if (result.ok && result.data) {
      console.log('[NOEST] ✅ Order created via API:', result.data.tracking);
      return {
        ok: true,
        data: {
  ...
  console.log('[NOEST] 🎭 Demo mode: simulating order creation...');
