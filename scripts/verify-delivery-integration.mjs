...
assert.match(providerSource, /workflow payloads in more than one shape/);
...
assert.match(settingsSource, /whatsappConfirmation:\s*false/);
...
assert.match(checkoutUiSource, /settings\.whatsappConfirmation/);
...
assert.match(adminSettingsSource, /return \{ \.\.\.current, noest: true, zrexpress: true \}/);
assert.match(adminSettingsSource, /تأكيد الطلب عبر WhatsApp/);
