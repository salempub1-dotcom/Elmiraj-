...
import { createOrder, getWilayas, getCommunes, getDesks, getWilayaCodeFromDeskCode, type NoestWilaya, type NoestCommune, type NoestDesk, type CreateOrderResult } from './services/noestApi';
...
// Admin auth is handled server-side via /api/auth — no credentials in client code
...
// generateTracking removed — tracking numbers come ONLY from NOEST API
...
function StoreApp({
  ...
  const handlePlaceOrder = async () => {
    ...
    // ── Send to NOEST — this is the ONLY source of truth ──
    ...
    // ── If NOEST failed → STOP. No fake tracking. No local order. ──
    ...
    // ── NOEST confirmed — extract REAL tracking from NOEST ──
  ...
  const resetCheckout = () => { setCheckoutOpen(false); setOrderPlaced(false); setCurrentOrder(null); setCustomerName(''); setCustomerPhone(''); setCustomerWilayaId(''); setCustomerWilayaLabel(''); setCustomerAddress(''); setCommune(''); setDeliveryType('home'); setSelectedOffice(''); setNoestCommunes([]); setOrderError(null); };
        ...
        <section className="bg-gradient-to-br from-emerald-900 via-emerald-800 to-emerald-700 text-white py-20 px-4">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="flex flex-wrap justify-center gap-4 mb-8">
              {(['تحضيري', 'ابتدائي', 'متوسط'] as const).map(cat => (<button key={cat} onClick={() => { setSelectedCategory(cat); setActiveSection('products'); }} className="bg-white/20 hover:bg-white/30 border border-white/40 text-white px-6 py-3 rounded-xl font-bold transition-all">{catEmoji(cat)} {cat}</button>))}
            </div>
        ...
        <section className="py-12 px-4 bg-gray-50">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-wrap justify-center gap-3 mb-8">
              {['الكل', 'تحضيري', 'ابتدائي', 'متوسط'].map(cat => (<button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-5 py-2.5 rounded-xl font-bold transition-all ${selectedCategory === cat ? 'bg-emerald-700 text-white shadow-md' : 'bg-white text-gray-600 hover:bg-emerald-50 border border-gray-200'}`}>{catEmoji(cat)} {cat}</button>))}
            </div>
...
function AdminApp({
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      setAdminLoginError('خطأ في الاتصال بالخادم، يرجى المحاولة لاحقاً');
    } finally {
