...
function NoestStatusCard({ showToast }: { showToast: (msg: string, type?: 'success' | 'error' | 'info') => void }) {
  ...
  const runDiagnose = async () => {
    ...
    if (result.ok) {
      ...
      showToast('✅ تم الفحص بنجاح', 'success');
    } else {
...
function AdminApp({
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Fallback to client-side auth (demo/static mode)
      console.log('[AUTH] Server auth unavailable, using demo fallback');
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-emerald-900 via-emerald-800 to-emerald-700 flex items-center justify-center p-4" dir="rtl">
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-md">
          ...
          <div className="mt-6 bg-emerald-50 rounded-xl p-4 text-center"><p className="text-xs text-emerald-600">🔒 الوصول مخصص للمسؤولين فقط</p><p className="text-xs text-gray-400 mt-1">تجريبي: admin / admin123</p></div>
        </div>
      </div>
    );
  }
